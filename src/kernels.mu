...
T trace(const std::vector<T>& h_input, size_t rows, size_t cols) {
  ...
  T* d_input;
  T* d_partial;
  musaMalloc(&d_input, rows * cols * sizeof(T));
  musaMalloc(&d_partial, n * sizeof(T));  // 修复：d_input -> d_partial
  ...
  musaMemcpy(h_partial.data(), d_partial, n * sizeof(T), musaMemcpyDeviceToHost);
...
__global__ void flashAttentionKernel(
  ...
  int kv_stride_s = kv_heads * head_dim;  // 修复：+ -> *
  ...
  // Causal mask 边界
  int s_end = src_seq_len;
  if (is_causal && t + 1 < src_seq_len) {
    s_end = t + 1;
  }
  ...
  for (int s = 0; s < s_end; s++) {
    ...
    // 1. 协作加载 K 到共享内存
    if (d < head_dim) {
      K_shared[d] = toFloat(K[kv_offset + d]);
    }
    __syncthreads();
  ...
  // 最终输出 = 加权和 / 分母
  if (d < head_dim) {
    int o_idx = idx4d(b, t, h, d, tgt_seq_len, query_heads, head_dim);
...
void flashAttention(const std::vector<T>& h_q, const std::vector<T>& h_k,
  ...
  int threads_per_block = ((head_dim + 31) / 32) * 32;
